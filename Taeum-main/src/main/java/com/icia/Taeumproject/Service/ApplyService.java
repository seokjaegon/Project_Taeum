...
//transaction 관련
	...
	public void updateApplyStatusWithNodeList(SearchDto sdto, int m_id, int page, int size, Model model) {
		...
		int pageSize = 3; // 페이지 목록 크기
		int totalPages = (int) Math.ceil((double) applyCnt / size);


		// 현재 페이지를 중심으로 시작 페이지와 끝 페이지 계산
		int prevPage = Math.max(1, page - 1);
		int nextPage = Math.min(totalPages, page + 1);
		
		int startPage = Math.max(1, page - 1); // 페이지 목록 시작 페이지
		int endPage = Math.min(totalPages, startPage + 2); // 페이지 목록 끝 페이지

		if (startPage == 1) { // 시작 페이지가 1인 경우
		    endPage = Math.min(totalPages, startPage + 2); // 페이지 목록 끝 페이지를 조정
		}
				...
				if (apply.getA_STARTADRESS() != null && !apply.getA_STARTADRESS().isEmpty()) {
					...
					if (startPoint != null) {
						...
						maServ.insertServ(apply.getM_ID(), apply.getA_STARTADRESS(), startPoint, kind, rttr,
								apply.getA_DATE(), apply.getM_NAME(), apply.getM_PHONE(), apply.getA_NAME(),
								apply.getA_CONTENTS(), apply.getA_ID());
